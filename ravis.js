...
async function loadRavisConfig() {
  try {
    const res = await fetch('https://ayush-brief-production.up.railway.app/api/admin/config', {
...
class Ravis {
  ...
  async speak(text) {
    ...
    try {
      const r = await fetch('https://ayush-brief-production.up.railway.app/api/voice/speak', {
        method: 'POST',
  ...
  async generateVoicesForCategory() {
    ...
    try {
      const res = await fetch('https://ayush-brief-production.up.railway.app/api/admin/generate-voices', {
        method: 'POST',
  ...
  async applyVoiceEdit(instruction) {
    ...
    try {
      const res = await fetch('https://ayush-brief-production.up.railway.app/api/admin/regenerate-voice', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-key': 'dawnbrief2026' },
        body: JSON.stringify({
          story_id: story.id,
  ...
  async finishKhatarnakSelect() {
    ...
    try {
      const res = await fetch('https://ayush-brief-production.up.railway.app/api/admin/generate-khatarnak-voices', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-admin-key': 'dawnbrief2026' },
        body: JSON.stringify({ story_ids: ids })
      });
